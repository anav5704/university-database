...
SELECT 
    ...
    EQ.QUALIFICATION_CODE IS NULL
FROM CANDIDATE AS C
INNER JOIN PLACEMENT AS P ON C.CANDIDATE_CODE = P.CANDIDATE_CODE
INNER JOIN JOB_OPENING AS JO ON P.JOB_OPENING_CODE = JO.JOB_OPENING_CODE
INNER JOIN QUALIFICATION AS Q ON JO.QUALIFICATION_CODE = Q.QUALIFICATION_CODE
LEFT JOIN EDUCATION AS EQ ON EQ.CANDIDATE_CODE = C.CANDIDATE_CODE AND EQ.QUALIFICATION_CODE = Q.QUALIFICATION_CODE
WHERE EQ.QUALIFICATION_CODE IS NULL
ORDER BY C.CANDIDATE_LNAME;
...
SELECT 
    ...
    c.CANDIDATE_LNAME,
    s.SESSION_FEE,
    COALESCE(e.ENROLLMENT_FEE_PAYMENT, 0) AS "Amount_Paid",
    (s.SESSION_FEE - COALESCE(e.ENROLLMENT_FEE_PAYMENT, 0)) AS "Balance",
    e.ENROLLMENT_DATE,
    CASE 
...
FROM CANDIDATE c
INNER JOIN ENROLLMENT e ON c.CANDIDATE_CODE = e.CANDIDATE_CODE
INNER JOIN SESSION s ON e.SESSION_CODE = s.SESSION_CODE
WHERE 
...
SELECT 
    comp.COMPANY_NAME,
    jo.JOB_OPENING_DESCRIPTION,
    jo.JOB_OPENING_AVAILABILITY_STATUS,
    q.QUALIFICATION_DESCRIPTION,
    c.CANDIDATE_FNAME,
    c.CANDIDATE_LNAME,
    e.EDUCATION_EARNED_DATE,
    COALESCE(placement_count.total_placements, 0) as "Number of Successful Placements"
FROM COMPANY comp
INNER JOIN JOB_OPENING jo ON comp.COMPANY_CODE = jo.COMPANY_CODE
...
INNER JOIN CANDIDATE c ON e.CANDIDATE_CODE = c.CANDIDATE_CODE
