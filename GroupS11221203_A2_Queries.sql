...
-- Query 1 --
-- List of all candidates and their qualifications --
...
-- Query 2 --
-- List of companies that have employed candidates -- 
...
-- Query 3 --
-- List of candidate and comapny details with matching qualifications --
...
SELECT CANDIDATE.CANDIDATE_CODE, CANDIDATE.CANDIDATE_FNAME, CANDIDATE.CANDIDATE_LNAME, COURSE.COURSE_DESCRIPTION, SESSION.SESSION_START_DATE,
SESSION.SESSION_FEE, ( SELECT SUM(SESSION.SESSION_FEE)
FROM SESSION
INNER JOIN COURSE ON SESSION.COURSE_CODE = COURSE.COURSE_CODE
WHERE COURSE.QUALIFICATION_CODE = JOB_OPENING.QUALIFICATION_CODE ) AS TOTAL_COST
FROM CANDIDATE
INNER JOIN PLACEMENT ON CANDIDATE.CANDIDATE_CODE = PLACEMENT.CANDIDATE_CODE
INNER JOIN JOB_OPENING ON PLACEMENT.JOB_OPENING_CODE = JOB_OPENING.JOB_OPENING_CODE
INNER JOIN EDUCATION ON CANDIDATE.CANDIDATE_CODE = EDUCATION.CANDIDATE_CODE
...
-- Query 5 --
-- List of candidates who haven't paid their fees in full for the first quarter --
...
WHERE E.ENROLLMENT_FEE_PAYMENT < S.SESSION_FEE AND E.ENROLLMENT_DATE >= '2025-01-01' AND E.ENROLLMENT_DATE <= '2025-03-31';

-- Query 6 --
-- Count successful placements by company and job opening --
SELECT C.COMPANY_NAME AS Company, J.JOB_OPENING_DESCRIPTION AS JOB_OPENING, Q.QUALIFICATION_DESCRIPTION AS REQUIRED_QUALIFICATION,
COUNT(P.CANDIDATE_CODE) AS SUCCESSFUL_PLACEMENTS
FROM COMPANY AS C
INNER JOIN JOB_OPENING AS J ON C.COMPANY_CODE = J.COMPANY_CODE
INNER JOIN QUALIFICATION AS Q ON J.QUALIFICATION_CODE = Q.QUALIFICATION_CODE
LEFT JOIN PLACEMENT AS P ON J.JOB_OPENING_CODE = P.JOB_OPENING_CODE
GROUP BY C.COMPANY_NAME, J.JOB_OPENING_DESCRIPTION, Q.QUALIFICATION_DESCRIPTION
ORDER BY C.COMPANY_NAME, J.JOB_OPENING_DESCRIPTION;

-- Query 7 --
-- Qualifications needed for openings with avaerage pay --
